        ...
        if os.path.isfile(os.path.join(directory, "{}.csv".format(arr_files[i]))):
            ...
            if len(data) != 0:
                ...
                range_months_file = [
                    i
                    for i in range(
                        int(first_date_in_data[1]), int(last_date_in_data[1]) + 1
                    )
                ]
                ...
                (
                    ...
                    dates_before_range,
                    dates_after_range,
                ) = start_end_dates_to_csv_row(list_date, start_date, end_date)
                if dates_before_range:
                    ...
                    for j in range(
                        min(e + 1, len(list_date) - 1), min(s - 1, len(list_date) - 1)
                    ):
                        ...
                        if list_date[j][2] != list_date[j - 1][2]:
                            ...
                            if first_date_in_data[2] == last_date_in_previous_data[2]:
                                ...
                                list_reviews.append(
                                    [
                                        analyze_reviews(data["content"][k:j]),
                                        list_date[j - 1],
                                    ]
                                )
    ...
    for i in range(len(list_reviews)):
        ...
        if check_date_between_dates(date_review, time_start, time_end):
            ...
            if keyword in list_reviews[i][0]:
                list_time_keyword[
                    calc_dist_between_dates(date_review, time_start)
                ] = list_reviews[i][0][keyword]
                print(
                    list_time_keyword[calc_dist_between_dates(date_review, time_start)]
                )
                print(i)
...
def keyword_recommendation(time_start, time_end, list_reviews):
    ...
    for i in range(len(list_reviews)):
        day_of_review = "/".join(
            [list_reviews[i][1][1], list_reviews[i][1][2], list_reviews[i][1][0]]
        )
    ...
    list_keywords_negative_reviews = list_with_same_indexes_to_one_index(
        list_keywords_negative_reviews
    )
    list_keywords_positive_reviews = list_with_same_indexes_to_one_index(
        list_keywords_positive_reviews
    )

    list_keywords_negative_reviews = sorted(
        list_keywords_negative_reviews, key=lambda x: x[1]
    )
    list_keywords_positive_reviews = sorted(
        list_keywords_positive_reviews, key=lambda x: x[1]
    )
...
def check_date_between_dates(date, start_date, end_date):
    ...
    if date_string <= end_date_string and date_string >= start_date_string:
        return True
    else:
        return False
        ...
        if os.path.isfile(os.path.join(directory, "{}.csv".format(arr_files[i]))):
            ...
            if len(data) != 0:
                ...
                for j in range(len(data["content"])):
                    ...
                    date = "{}/{}/{}".format(
                        date_current[1], date_current[2], date_current[0]
                    )
                    # print(data["content"][j])
                    if keyword in str(data["content"][j]) and check_date_between_dates(
...
def latest_first_date_earliest_second_date(
    first_date_1, first_date_2, second_date_1, second_date_2
):
